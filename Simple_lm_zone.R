...
do_for_ab <- function(out_file_name, ab, df, org_name, file_category) {
  ...
  model_filename <- paste0("code/lm/simple_lm_zone/models/", file_category, "_", org_name, "_", ab, ".rds")
...
read_dir_inner <- function(category, category_types) {
  ...
  for(category_type in category_types) {
    ...
    write.csv(x=df_category, file=paste0("code/lm/simple_lm_zone/", category_type, ".csv"))
  }
}

read_dir_inner("zone", c("North-east", "North", "South", "East", "West", "Central"))
#read_dir_inner("zone", c("North-east"))