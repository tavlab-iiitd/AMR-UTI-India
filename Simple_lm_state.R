...
do_for_ab <- function(out_file_name, ab, df, org_name, file_category) {
  ...
  model_filename <- paste0("code/lm/simple_lm_state/models/", file_category, "_", org_name, "_", ab, ".rds")
...
read_dir_inner <- function(category, category_types) {
  ...
  for(category_type in category_types) {
    ...
    write.csv(x=df_category, file=paste0("code/lm/simple_lm_state/", category_type, ".csv"))
  }
}

read_dir_inner("state", c("Rajasthan", "Manipur", "Jammu and Kashmir", "Uttar Pradesh", "Madhya Pradesh", "Telangana", "Karnataka","Assam","Tamil Nadu","West Bengal", "Maharashtra", "Delhi", "Puducherry"))
