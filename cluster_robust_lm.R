...
#Models
#Simple linear model on national data
model1 <- lm(trend ~ seq_num, data = filtered_data)
summary_model1 <- summary(model1)
summary(model1)
summary_model1$r.squared

#Models considering interaction effects
model2 <- lm(trend ~ seq_num + factor(hospital_code), data = decomposed_data)
model3 <- lm(trend ~ seq_num * factor(hospital_code), data = decomposed_data)

summary_model3 <- summary(model3)
summary_model3$r.squared

plot_model(model3, type = "pred", terms = c("seq_num", "hospital_anonym"))

#Robust standard errors on model 3
robust_se <- coeftest(model3, vcov = vcovHAC(model3, type = "HC", cluster = "hospital_code", group = decomposed_data$hospital_code))
file <- paste0("robust_se_", antibiotic, "_ecoli.csv")
write.csv(robust_se, file)
print(robust_se)
