...
do_for_ab <- function(out_file_name, ab, df, org_name) {
  ...
  model_filename <- paste0("code/lm/simple_lm_gender/models/", org_name, "_", ab, ".rds")
...
get_trend_p_value <- function(s) {
  p <- pf(s$fstatistic[1],              # Applying pf() function
          s$fstatistic[2],
          s$fstatistic[3],
          lower.tail = FALSE)
...
handle_file_inner <- function(csv_file, output_map, output_cat, categories) {
  ...
  org_name <- NULL
  for(category in categories) {
    category_index = unlist(gregexpr(category, csv_file))[1]
    if(category_index != -1) {
  ...
  for(ab in abs) {
    ...
    s <- do_for_ab(out_file_name, ab, dat, org_name)
...
read_dir_inner <- function(category, category_types) {
  ...
  for(category_type in category_types) {
    ...
    write.csv(x=df_category, file=paste0("code/lm/simple_lm_gender/", category_type, ".csv"))
  }
}

read_dir_inner("gender", c("Male", "Female"))
